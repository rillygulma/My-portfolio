...
function Card({ title, des, icon }) {
  return (
    <div className="w-full px-6 sm:px-8 md:px-12 h-auto py-8 sm:py-10 rounded-lg shadow-shadowOne flex items-center
    ...
    hover:from-black hover:to-[#1e2024] transition-colors duration-100 group">
      <div className="h-auto overflow-hidden">
        <div className="flex h-full flex-col gap-6 sm:gap-8 translate-y-16  
          group-hover:translate-y-0 transition-transform duration-500">
          <div>
            <span className="text-4xl sm:text-5xl text-designColor">{icon}</span>
          </div>
          <div className="flex flex-col gap-4 sm:gap-6">
            <h2 className="text-xl sm:text-2xl font-titleFont font-bold text-gray-300">
              {title}
            </h2>
